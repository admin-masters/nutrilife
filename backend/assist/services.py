...
def approve_all(org: Organization, actor: User | None) -> Tuple[ApprovalBatch, int]:
    ...
    batch = ApprovalBatch.objects.create(organization=org, created_by=actor, method=ApprovalBatch.Method.ALL_PENDING, n_selected=len(pending))
    approved_count = 0
    for app in pending:
        ...
        # Ensure Enrollment
        if not hasattr(app, "enrollment"):
            Enrollment.create_for_approved(app, actor)
        approved_count += 1
...
def approve_top_n(org: Organization, n: int, actor: User | None) -> Tuple[ApprovalBatch, int, int]:
    ...
    batch = ApprovalBatch.objects.create(organization=org, created_by=actor, method=ApprovalBatch.Method.TOP_N_ALPHA, n_selected=len(to_approve))
    approved_count = 0
    skipped = 0
